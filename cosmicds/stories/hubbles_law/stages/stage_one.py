...
from ..utils import age_in_gyr, extend_tool, format_fov, format_measured_angle, line_mark, vertical_line_mark
...
class StageOne(VuetifyTemplate, HubListener):
    ...
    def __init__(self, session, *args, **kwargs):
        ...
        # self.app.load_data(str(data_dir / "galaxy_data.csv"),
        #     label='galaxy_data')
        # self.app.load_data(str(data_dir / "Hubble 1929-Table 1.csv"),
        #     label='Hubble 1929-Table 1')
        # self.app.load_data(str(data_dir / "HSTkey2001.csv"),
        #     label='HSTkey2001')
        ...
        #     self.app.load_data(str(output_dir / f"{dataset}.csv"), label=dataset)
        ...
        spectrum_viewer = self.app.new_data_viewer(
            SpectrumView, data=None, show=False)

        self.app.load_data(str(data_dir / "spectra" / 
        ...
        hub_viewers = [self.app.new_data_viewer(BqplotScatterView, data=None, show=False) for _ in range(6)]
        ...
        for component in class_data.components:
            ...
            self.app.add_link(class_data, field, all_data, field)
        ...
        for component in class_data.components:
            ...
            if field in student_data.component_ids():
                self.app.add_link(student_data, field, class_data, field)
        ...
        self.components = {'c-galaxy-table': Table(self.session, measurement_data, glue_components=self._galaxy_table_components,
        ...
        default_style_path = str(Path(__file__).parents[1] / "data" /
                                        "styles" / "default_scatter.json")
        comparison_style_path = str(Path(__file__).parents[1] / "data" /
                                        "styles" / "comparison_scatter.json")
        prodata_style_path = str(Path(__file__).parents[1] / "data" /
        ...
        self.app.add_link(hubble1929, 'Distance (Mpc)', hstkp_data, 'Distance (Mpc)')
        self.app.add_link(hubble1929, 'Tweaked Velocity (km/s)', hstkp_data, 'Velocity (km/s)')
        self.app.add_link(hstkp_data, 'Distance (Mpc)', student_data, 'distance')
        self.app.add_link(hstkp_data, 'Velocity (km/s)', student_data, 'velocity')
        ...
        self.app.add_link(self.data_collection['HubbleSummary_Students'], 'age', self.data_collection['HubbleSummary_Classes'], 'age')
        self.app.add_link(self.data_collection['HubbleSummary_ClassSample'], 'age', self.data_collection['HubbleSummary_Classes'], 'age')

        # Scatter viewer used for the galaxy selection
        gal_viewer = self.app.new_data_viewer(
            BqplotScatterView, data=self.data_collection['galaxy_data'],
            show=False)

        # Histogram viewers for age distribution
        age_distr_viewers = [self.app.new_data_viewer(BqplotHistogramView, data=None, show=False) for _ in range(3)]
        ...
        self.app.set_subset_mode('replace')
    ...
    def data_collection(self):
        return self.app.data_collection
