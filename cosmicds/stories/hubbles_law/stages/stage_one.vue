<template>
  <v-container>
    <v-row>
      <v-col>
        <v-btn color="green" @click="select_galaxies()"
          >select 5 galaxies</v-btn
        >
        <v-btn @click="console.log(stage_state)">State</v-btn>
      </v-col>
    </v-row>
    <v-row>
      <v-col cols="12" lg="4">
        <c-select-galaxies-guidance v-if="stage_state.marker == 'sel_gal1'" />
      </v-col>
      <v-col cols="12" lg="8">
        <v-card
    ...
    <v-row>
      <v-col cols="12" lg="4">
        <c-choose-row-alert v-if="stage_state.marker == 'cho_row1'" />
      </v-col>
      <v-col cols="12" lg="8" class="galtable_column">
        <v-card
    ...
    <v-row>
      <v-col cols="12" lg="4">
        <c-spectrum-guidance v-if="stage_state.marker == 'mee_spe1'" />
        <c-restwave-alert v-if="stage_state.marker == 'res_wav1'" />
        <c-obswave-alert v-if="stage_state.marker == 'obs_wav1'" />
        <c-remaining-gals-alert v-if="stage_state.marker == 'rep_rem1'" />
        <c-nice-work-alert v-if="stage_state.marker == 'nic_wor1'" />
      </v-col>
      <v-col cols="12" lg="8">
        <v-card
          color="info"
          class="pa-1"
          v-if="
            stage_state.indices[stage_state.marker] >=
            stage_state.indices['mee_spe1']
          "
        >
    ...
    <v-row>
      <v-col cols="12" lg="8" offset-lg="4">
        <v-row>
          ...
          <v-col cols="3">
            <span :class="stage_state.waveline_set ? 'd-block' : 'd-none'">
              <!-- FORM DIALOG as template for reflections/MC -->
