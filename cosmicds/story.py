...
from .data_management import CLASS_DATA_LABEL, CLASS_SUMMARY_LABEL, \
    SDSS_DATA_LABEL, STATE_TO_MEAS, STATE_TO_SUMM, \
...
class HubblesLaw(Story):
    ...
    def __init__(self, *args, **kwargs):
        ...
        galaxies = requests.get(
            f"{API_URL}/{HUBBLE_ROUTE_PATH}/galaxies").json()
        galaxies_dict = {k: [x[k] for x in galaxies] for k in galaxies[0]}
        galaxies_dict["name"] = [x[:-len(self.name_ext)] for x in
                                 galaxies_dict["name"]]
        ...
        all_json = requests.get(
            f"{API_URL}/{HUBBLE_ROUTE_PATH}/all-data").json()
        ...
        all_data = Data(
            ...
            **{STATE_TO_MEAS.get(k, k): [x[k] for x in all_measurements] for k
               in all_measurements[0]}
        )
        HubblesLaw.prune_none(all_data)
        self.data_collection.append(all_data)

        all_student_summ_data = self.data_from_summaries(all_student_summaries,
                                                         "all_student_summaries")
        all_class_summ_data = self.data_from_summaries(all_class_summaries,
                                                       "all_class_summaries")
        self.data_collection.append(all_student_summ_data)
        self.data_collection.append(all_class_summ_data)
        for comp in ['age', 'H0']:
            self.app.add_link(all_student_summ_data, comp, all_class_summ_data,
                              comp)
    ...
    def prune_none(data):
        ...
        pruned_components = {comp.label: [data[comp][x] for x in keep] for comp
                             in data.main_components}
    ...
    def galaxy_info(self, galaxy_ids):
        ...
        return {sdss['id'][index]: {comp.label: sdss[comp][index] for comp in
                                    components} for index in indices}

    def data_from_measurements(self, measurements):
        for measurement in measurements:
            measurement.update(measurement.get("galaxy", {}))
        components = {
            STATE_TO_MEAS.get(k, k): [measurement.get(k, None) for measurement
                                      in measurements] for k in
    ...
    def data_from_summaries(self, summaries, label=None):
        components = {
            STATE_TO_SUMM.get(k, k): [summary.get(k, None) for summary in
                                      summaries] for k in self.summary_keys}
    ...
    def fetch_measurement_data_and_update(self, url, label, prune_none=False,
    ...
    def fetch_student_data(self):
        ...
        self.fetch_measurement_data_and_update(student_meas_url,
    ...
    def fetch_class_data(self):
        def check_update(measurements):
            last_modified = max(
                [datetime.fromisoformat(x["last_modified"][:-1]) for x in
                 measurements])
        ...
        self.fetch_measurement_data_and_update(class_data_url,
                                               CLASS_DATA_LABEL,
                                               prune_none=True,
                                               check_update=check_update)
        self.update_summary_data(CLASS_DATA_LABEL, CLASS_SUMMARY_LABEL,
                                 "student_id")
