...
class Table(VuetifyTemplate, HubListener):
    ...
    def __init__(self, session, data, tools=None, item_filter=None, *args, **kwargs):
        ...
        self.tool_functions = {}
        self._subset = kwargs.get(
            "subset", None
        )  # Can be either a subset or subset group
        ...
        if tools:
            for tool in tools:
                ...
                del tool["activate"]
                self.tools = {tool_id: tool, **self.tools}
        ...
        self._data_collection_delete_filter = (
            lambda message: message.data == self._glue_data
        )
        self._data_update_filter = (
            lambda message: message.data == self._glue_data
            and message.attribute in self._glue_components
        )
        ...
        def subset_changed_filter(message):
            if self._is_subset_group:
                return (
                    message.subset in self._subset.subsets
                    and message.subset.data == self._glue_data
                )
            else:
        ...
        self.hub.subscribe(
            ...
            filter=self._data_collection_delete_filter,
        )
        self.hub.subscribe(
            self,
            DataUpdateMessage,
            handler=self._on_data_updated,
            filter=self._data_update_filter,
        )
        self.hub.subscribe(
            self,
            NumericalDataChangedMessage,
            handler=self._on_data_updated,
            filter=self._data_changed_filter,
        )
        self.hub.subscribe(
            self,
            SubsetUpdateMessage,
            handler=self._on_subset_updated,
            filter=self._subset_changed_filter,
        )
    ...
    def subset_state_from_selected(self, selected):
        ...
        if keys:
            state = np.bitwise_or.reduce(
                [self._glue_data.id[self.key_component] == x for x in keys]
            )
        else:
            state = SubsetState()
        return state
    ...
    def _populate_table(self):
        ...
        self.headers = [
            {"text": name, "value": component}
            for name, component in zip(
                self._glue_component_names, self._glue_components
            )
        ]
        ...
        self.items = [
            ...
            if self.item_filter(
                item := {
                    component: self._transform(component)(row[component])
                    for component in self._glue_components
                }
            )
        ]
    ...
    def _new_subset(self):
        ...
        if self.use_subset_group:
            subset = self.data_collection.new_subset_group(
                label=self._subset_label, subset_state=state, color=self.subset_color
            )
        else:
            subset = self._glue_data.new_subset(
                label=self._subset_label, subset=state, color=self.subset_color
            )
